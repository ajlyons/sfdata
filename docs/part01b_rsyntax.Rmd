...
```{r computing, echo=TRUE, eval=FALSE}
2 + 2     # add numbers
3 ^ 4     # powers
sqrt(49) # functions
```

Notice that **white space is generally ignored**.

<div class="tryit">
Open up RStudio and compute the following:
```
87 * 45
```
</div>

# Save the results of expressions to variables (objects)

A key technique in all programming languages is to store values in the form of variables (also called objects). In R, you can use the '<tt>=</tt>' or '<tt><-</tt>' operators to do this.
...
<div class="tryit">
Using the <tt>AirPassengers</tt> dataset (monthly totals of international airline passengers in thousands from 1949 to 1960), write expressions that return:
...
<div class="tryit">
Using the <tt>AirPassengers</tt> dataset again, write an expression that returns the elements greater than 400. 

How many months during this time period were there more than 400,000 international airline passengers? 
</div>
