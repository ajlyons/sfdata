...
toc_link <- local({
  function(title, html_fn, type=c("slides", "exercises")[1], link=TRUE) {
    if (link) {
      atag1 <- paste0("<a target=\"_blank\" class=\"", type, "\" href=\"", type, "/", html_fn, "\">")
      atag2 <- "</a>"
    } else {
      atag1 <- ""
      atag2 <- ""
    }
    html_snip <- paste0("<li class=\"", type, "\">", atag1, title, atag2, "</li>")
    return(html_snip)
  }
})
...
prep4id <- local({
 function(x) {
   ## Removes all punctuation, converts space to dashes, and lower case
   return(tolower(gsub("[[:space:]]", "-", gsub('[[:punct:]]', "", x))))
 }
})
